// Land of Fire · Adaptador de FighterInput a FighterCommandSource.
// Va en FighterRoot junto con FighterInput; un dummy puede usar otra fuente u omitirla.
using System;
using UnityEngine;
...
namespace LandOfFire.BunnyStep
{
    ...
    public sealed class PlayerCommandSource : FighterCommandSource
    {
        ...
        public override bool TryReadAttack(out AttackCommand command)
        {
            command = default(AttackCommand);
            return input != null && input.isActiveAndEnabled && input.TryReadAttack(out command);
        }
