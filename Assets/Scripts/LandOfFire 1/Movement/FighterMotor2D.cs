...
namespace LandOfFire.BunnyStep
{
    ...
    public sealed class FighterMotor2D : MonoBehaviour
    {
        ...
        [Tooltip("Opcional: al añadir FighterCombat2D habilita el golpe L.")]
        ...
        public Vector2 BodyPosition => body != null ? body.position : (Vector2)transform.position;
        ...
        private void Awake()
        {
            ...
            if (combat == null) combat = GetComponent<FighterCombat2D>();
        ...
        {
            if (!ready) return;
            Machine.Reset(); accumulated = 0; tick = 0; poseTick = 0; hitstopRemaining = 0;
        ...
        private void FixedUpdate()
        {
            ...
            while (accumulated + 1e-9 >= TickSeconds)
            {
                ...
                if (hitstopRemaining > 0)
                {
                    ...
                    pausedThisStep = true;
                    tick++;
                    continue; // Ni Bunny, ni golpe, ni Hurt gastan ticks durante hitstop.
                }
                ...
                if (hasCommands)
                {
                    // Si L y Forward llegan en el mismo tick desde Idle, L tiene prioridad.
                    while (commands.TryReadAttack(out AttackCommand attackCommand))
                    {
                        AttackMoveData move = combat != null ? combat.MoveFor(attackCommand) : null;
                        if (Machine.TryStartAttack(move))
                        {
                            combat.BeginAttack(move);
                            delta = 0; // Un cancel detiene desplazamiento voluntario anterior.
                        }
                    }
                ...
                if (combat != null) combat.ResolveTick(Machine, facing);
                tick++;
            }
            // Proponemos velocidad. Physics 2D resuelve contactos, sin escribir transform.position.
            bool stopped = hitstopRemaining > 0 || (pausedThisStep && !resumedThisStep);
            body.constraints = stopped ? RigidbodyConstraints2D.FreezeAll : GroundConstraints;
            SetVelocity(stopped ? Vector2.zero : new Vector2(delta / Time.fixedDeltaTime, 0));
        ...
        /// <summary>Congela el tiempo lógico del luchador; el golpe ya aceptado no se reevalúa.</summary>
        public void ApplyHitstop(int ticks)
        {
            if (!ready || ticks <= 0) return;
            hitstopRemaining = Mathf.Max(hitstopRemaining, ticks);
            body.constraints = RigidbodyConstraints2D.FreezeAll;
            SetVelocity(Vector2.zero);
        }
